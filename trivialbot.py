...
class Question():
    ...
    def __str__(self):
        return f'{self.category} ({self.length} characters)\n==========================\n{self.text}'
...
def handle_msg(update, context):
    ...
    elif (txt == '!ask' and update.message.from_user.id == TELEGRAM_ADMIN_ID):
        get_question(update, context)
    elif (txt == '!clue' and current_question):
        clue = current_question.clue()
        if clue != None:
            context.bot.send_message(chat_id=update.message.chat_id, text=clue)
    elif current_question:
        ratio = current_question.check(txt)
        print(f'The current ratio is {ratio}')
        if (ratio > 0.85):
            usr = update.message.from_user
            usrid = str(usr.id)
            n = db.zadd('ranking', {usrid: '1'}, incr=True)
            # Our user didn't exist in the database
            if n == 1.0:
            ...
            msg = f'Right, {name}! The answer was {answ}'
            context.bot.send_message(chat_id=update.message.chat_id, text=msg)

            print_ranking(update, context)
            get_question(update, context)
        elif ratio > 0.75:
            context.bot.send_message(chat_id=update.message.chat_id, text="Someone's very close!")
